...
export function DownloadsListRenderItem({jobDetails, onPress}: Props) {
  ...
  const getBackgroundColor = useCallback((): ViewStyle['backgroundColor'] => {
    switch (status) {
      case 'active':
        return Color.rgb(systemCyan).alpha(0.1).toString();
      case 'succeeded':
        return Color.rgb(systemGreen).alpha(0.1).toString();
      case 'failed':
        return Color.rgb(systemRed).alpha(0.1).toString();
      case 'queued':
        return Color.rgb(systemIndigo).alpha(0.1).toString();
    }
  }, [status]);
