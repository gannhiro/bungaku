...
export type CacheChapterProps = {
  ...
  isDataSaver: boolean;
  callback: (
    pages: {pagePromise?: Promise<FS.DownloadResult>; path: string}[],
...
const processJobsQueue = createAsyncThunk<void, void, AppAsyncThunkConfig>(
  ...
  async (_, {getState, dispatch}) => {
    ...
    const maxConcurrentDownloads = state.userPreferences.maxConcurrentDownloads;

    let activeDownloads = state.jobs.activeDownloads;

    while (activeDownloads < maxConcurrentDownloads && queuedJobs.length > 0) {
      activeDownloads++;
      dispatch(addOneToActiveDownloadNumber());
      const nextJob = queuedJobs.shift();
      if (nextJob) dispatch(downloadChapterJob(nextJob));
    }
  },
);
...
const downloadChapterJob = createAsyncThunk<void, JobPayload, AppAsyncThunkConfig>(
  ...
  async (jobPayload, {dispatch}) => {
    ...
    if (!jobDb) return console.error(`Job not found in DB for download: ${jobId}`);
    ...
    try {
      ...
      if (chapterIsCached) {
        ...
        const chapterDownloadDirExists = await FS.exists(chapterDownloadDirectory);

        if (!chapterDownloadDirExists) await FS.mkdir(chapterDownloadDirectory);
      ...
      dispatch(removeOneFromActiveDownloadNumber());
...
export const cacheChapterJob = createAsyncThunk<void, CacheChapterProps, AppAsyncThunkConfig>(
  'jobs/cacheChapter',
  async props => {
    const {chapter, mangaId, isDataSaver, callback} = props;
    ...
    callback(pageDownloads, chapterData);
  },
);
...
export const jobsSlice = createSlice({
  ...
  reducers: {
    ...
    addOneToActiveDownloadNumber: state => {
      state.activeDownloads++;
    },
    removeOneFromActiveDownloadNumber: state => {
      state.activeDownloads--;
    },
  },
});
...
export const {
  ...
  removeJob,
  setJobs,
  addOneToActiveDownloadNumber,
  removeOneFromActiveDownloadNumber,
} = jobsSlice.actions;
